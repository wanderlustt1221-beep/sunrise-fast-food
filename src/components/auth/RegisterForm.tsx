...
// On success: premium toast + redirect to homepage immediately.
...
import {
    ...
    Eye, EyeOff, ArrowRight, Loader2, Building2, Hash, Navigation,
} from "lucide-react";
...
function Field({
    ...
    autoComplete, icon, rightEl, required, colSpan, optional,
}: FieldProps) {
    return (
        <div className={`flex flex-col gap-1.5 ${colSpan ? "sm:col-span-2" : ""}`}>
            <label htmlFor={id} className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-[0.18em] text-stone-500">
                {label}
                {optional && <span className="rounded-full bg-stone-100 px-1.5 py-0.5 text-[9px] font-bold normal-case tracking-normal text-stone-400">optional</span>}
            ...
            <div className="relative flex items-center">
                ...
                <input
                    ...
                    required={required}
                    className="w-full rounded-xl border border-amber-200 bg-amber-50/40 py-2.5 pl-10 pr-10 text-sm font-medium text-stone-800 placeholder:text-stone-400 transition-all duration-200 focus:border-orange-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-200 hover:border-amber-300"
                />
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (missing.length) {
            toast.error("Please fill in all required fields ✱", {
                style: { border: "1.5px solid #fca5a5", borderRadius: "14px" },
            });
            return;
        }
        ...
        try {
            ...
            // ── Celebratory premium success toast ────────────────────────
        ...
        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <Field
                    ...
                    icon={<Calendar size={14} strokeWidth={2} />} optional
                />
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                />
                <Field
                    id="reg-password" label="Password" type={showPass ? "text" : "password"}
